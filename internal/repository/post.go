...
func (p *postQuery) CreatePost(post model.Post) (int64, error) {
	res, err := p.db.Exec("INSERT INTO posts (title, message, user_id, category, like, dislike, born) VALUES(?,?,?,?,?,?,?)", post.Title, post.Content, post.Author.ID, post.Category, 0, 0, post.CreateTime)
	if err != nil {
		return 0, err
	}
...
func (p *postQuery) GetPostById(postId int64) (model.Post, error) {
	row := p.db.QueryRow("SELECT post_id, title,  message, user_id, like, dislike, category, born FROM posts WHERE post_id = ? ", postId)
	var post model.Post
	if err := row.Scan(&post.ID, &post.Title, &post.Content, &post.Author.ID, &post.Like, &post.Dislike, &post.Category, &post.CreateTime); err != nil {
		return model.Post{}, err
	}
	return post, nil
}
